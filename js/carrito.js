...
function actualizarBotonCarrito() {
    let productos_carrito = cargarProductosCarrito();
    let contenido = `<button type="button" class="btn btn-warning position-relative"><img src="images/basket.svg" alt="Carrito" width="32"><span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">${productos_carrito.length}</span></button>`;
    ...
    if (document.getElementById("productos_seleccionados")) {
        ...
        if (productos.length == 0) {
            ...
            for (const producto of productos) {
                ...
                <td class='text-center'><button class='btn btn-warning' onclick='eliminarProducto(${producto.id});' title='Eliminar Producto'>-</button> <b>${producto.cantidad}</b> <button class='btn btn-warning' onclick='agregarProducto(${producto.id});' title='Agregar Producto'>+</button></td>
